...
{
	public class LessorRegisterScript : ABaseScript
	{
		...
		public LessorRegisterScript(ProEstateDatabase database) : base(database)
		{
		}

		public override string Name => "Реєстрація орендодавця";
		...
		public override void Execute(params object[] list)
		{
			...
			if (CheckFields())
			{
				if (CrownMessageBox.ShowInformation("Перед створенням нового запису перевірте правильність введених даних. Ви дійсно хочете створити новий запис?", Name, DialogButton.YesNo) == DialogResult.No)
					return;

				document.Enabled = false;
				Database.Lessees.InsertOnSubmit(new Lessor
				{
				...
				CrownMessageBox.ShowInformation("Запис було успішно створено.", Name);
				document.Close();
			}
			else
